<div class="container mx-auto px-6 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Управление требованиями к лицензиям</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Создание и управление требованиями к документам для лицензий</p>
        </div>
        @if($canCreate)
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select wire:model.live="filterRequired"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 transition-colors">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-blue-900/20 dark:hover:to-indigo-900/30 transition-all duration-150">
                        ...
                        <td class="px-4 py-4 whitespace-nowrap text-center">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-red-100 to-orange-100 dark:from-red-900/30 dark:to-orange-900/30 text-red-800 dark:text-red-300 border border-red-200 dark:border-red-700">
                        ...
                        <td class="px-4 py-4 text-center">
                            @php
                                $extensions = $requirement->allowed_extensions;
                            ...
                            @else
                            <span class="text-xs text-gray-400 dark:text-gray-500">Не указано</span>
                            @endif
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-center">
                            <span class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                {{ $requirement->max_file_size_mb }} МБ
                            </span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-center">
    ...
    <div class="mt-8 flex justify-center">
        {{ $requirements->links('pagination::livewire-tailwind') }}
    </div>
    @endif
    @else
    <div class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="flex flex-col items-center">
            <i class="fas fa-list-check text-4xl text-gray-300 dark:text-gray-600 mb-3"></i>
            <p class="text-gray-500 dark:text-gray-400 font-medium">Требования не найдены</p>
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200 dark:border-gray-700 relative" style="z-index: 9999;">
                <form wire:submit.prevent="createRequirement">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[85vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Создание требования к лицензии</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <!-- Category and Document -->
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Категория документа</label>
                                ...
                                <div>
                                    ...
                                    <select wire:model="documentId"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors"
                                            @if(empty($categoryId)) disabled @endif>
                                        <option value="">{{ empty($categoryId) ? 'Сначала выберите категорию' : 'Выберите документ' }}</option>
                                        @if(!empty($categoryId))
                                            @foreach($this->getDocumentsByCategory($categoryId) as $document)
                                            <option value="{{ $document->id }}">{{ $document->title_ru }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('documentId') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Обязательность *</label>
                                    <select wire:model="isRequired"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
                                        <option value="1">Обязательный</option>
                                        <option value="0">Необязательный</option>
                                    </select>
                                    @error('isRequired') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Макс. размер файла (МБ) *</label>
                                    <input type="number" step="0.1" wire:model="maxFileSizeMb"
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
                                    @error('maxFileSizeMb') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>

                            <!-- Allowed Extensions -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Разрешенные расширения файлов</label>
                                <div class="flex flex-wrap gap-2">
                                    @php
                                        ...
                                        $currentExtensions = $allowedExtensions;
                                        // Ensure it's an array
                                    ...
                                    @foreach($availableExtensions as $ext)
                                        <label class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border cursor-pointer transition-colors
                                            ...
                                            <input type="checkbox"
                                                   value="{{ $ext }}"
                                                   wire:model.live="allowedExtensions"
                                                   class="sr-only">
                                            <span>.{{ $ext }}</span>
                                        </label>
                                    @endforeach
                                </div>
                                @error('allowedExtensions') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200 dark:border-gray-700 relative" style="z-index: 9999;">
                <form wire:submit.prevent="updateRequirement">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[85vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Редактирование требования к лицензии</h3>
                        </div>
