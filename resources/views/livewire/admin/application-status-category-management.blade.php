<div class="container mx-auto px-6 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Категории статусов заявок</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Управление категориями статусов заявок на лицензирование</p>
        </div>
        @if($canCreate)
                    ...
                    @foreach($categories as $category)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <td class="px-4 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $category->id }}</span>
                            </td>
                            <td class="px-4 py-4">
                                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $category->title_ru }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $category->title_kk }}</div>
                                @if($category->title_en)
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $category->title_en }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-4 whitespace-nowrap">
                                <code class="text-xs bg-gray-100 dark:bg-gray-700 text-blue-600 dark:text-blue-400 px-2 py-1 rounded font-mono">{{ $category->value }}</code>
                            </td>
                            <td class="px-4 py-4">
                                @if(!empty($category->role_values))
                                    @php
                                        $categoryRoleValues = $category->role_values;
                                        if (is_string($categoryRoleValues)) {
                                            $categoryRoleValues = json_decode($categoryRoleValues, true) ?? [];
                                        }
                                    @endphp
                                    <div class="flex flex-wrap gap-1 justify-center">
                                        @foreach($categoryRoleValues as $roleValue)
                                            @php
                                ...
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-900/30 text-gray-800 dark:text-gray-400">
                                            <i class="fas fa-times-circle mr-1"></i>Неактивна
                                        </span>
                                    @endif
                                    @if($category->is_first)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">
                                            <i class="fas fa-play mr-1"></i>Первая
                                        </span>
                                    @endif
                                    @if($category->is_last)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200">
                                            <i class="fas fa-stop mr-1"></i>Последняя
                                        </span>
                                    @endif
                                </div>
                            </td>
    ...
    <div class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="flex flex-col items-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 font-medium">Категории статусов не найдены</p>
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200 dark:border-gray-700 relative" style="z-index: 9999;">
                <form wire:submit.prevent="createCategory">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[90vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Создание категории статуса</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Предыдущая категория</label>
                                    <select wire:model="catPreviousId"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
                                    ...
                                    @error('catPreviousId') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Следующая категория</label>
                                    <select wire:model="catNextId"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors">
                                        <option value="">Нет</option>
                                        @foreach($availableCategories as $cat)
                                            <option value="{{ $cat->id }}">{{ $cat->title_ru }}</option>
                                    ...
                                    @error('catNextId') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div class="flex items-center">
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        <input type="checkbox" wire:model="isFirst"
                                               class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:bg-gray-700 transition-colors">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Первая</span>
                                    </label>
                                </div>
                                ...
                                <div class="flex items-center">
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        <input type="checkbox" wire:model="isLast"
                                               class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:bg-gray-700 transition-colors">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Последняя</span>
                                    </label>
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('result') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200 dark:border-gray-700 relative" style="z-index: 9999;">
                <form wire:submit.prevent="updateCategory">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[90vh] overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Редактирование категории статуса</h3>
                        </div>
